...
// @Router       /did/create [get]
func DIDCreate(c *gin.Context) {
	...
	var didID int
	err = pool.QueryRow(context.Background(), "INSERT INTO dids (did_id, public_key_id, status, auth) VALUES ($1, $2, 'creating','pwj') RETURNING id", fmt.Sprintf("did:pwj:%s", hex.EncodeToString(id)), publicKeyID).Scan(&didID)
	if err != nil {
	...
	publicKeyDID := models.PublicKey{
		ID:         fmt.Sprintf("did:pwj:%s#keys-1", hex.EncodeToString(id)),
		Type:       "EcdsaSecp256k1VerificationKey2019",
		Controller: fmt.Sprintf("did:pwj:%s", hex.EncodeToString(id)),
		PublicKey:  hexutil.Encode(crypto.FromECDSAPub(publicKeyECDSA)),
	}

	did := models.DID{
		ID:      fmt.Sprintf("did:pwj:%s", hex.EncodeToString(id)),
